...
from architectures.layers.crf_as_rnn.crf_rnn_layer import crf_rnn_layer
...
class UNetCRF(object):

    def __init__(self, n_out, is_training, n_filters=64, use_bn=True, upsampling_mode='NN', name='UNet_CRF_as_RNN'):
        """
        ...
        # parameters for CRF as RNN (as in https://github.com/MiguelMonteiro/CRFasRNNLayer/blob/master/example.py):
        # Nota Bene: you must compile crf_as_rnn layer using:
        #   | SPATIAL_DIMS=2 (2D inputs)
        #   | INPUT_CHANNELS=self.n_out (input channels to the layer)
        #   | REFERENCE_CHANNELS=1 (input channels of 'incoming', e.g. input image)
        # Parameters below (for CRF as RNN) chosen according to the task of cardiac segmentation in the paper "Learning
        # to Segment Medical Images with Scribble-Supervision Alone", Can et al., MICCAI 2018
        self.theta_alpha = 160  # 50
        self.theta_beta = 3  # 25
        self.theta_gamma = 10  # 50
    ...
    def build(self, incoming, reuse=tf.AUTO_REUSE):
        ...
        with tf.variable_scope(self.name, reuse=reuse):
            ...
            self.prediction = self.build_output(decoder, reference_image=incoming)
    ...
    def build_output(self, decoder, reference_image):
        """ Output layers """
        # output linear

        with tf.variable_scope('logits'):
            logits = self._output_layer(decoder, n_channels_out=self.n_out, scope='output')
